...
from .schemes import (ProjectBody, ProjectResponse, ProjectListResponse, ProjectUpdatePath, ProjectDeletePath, ProjectDaysListResponse, ProjectDaysResponse)
from apps.project.application.services import ProjectCreateService, ProjectListService, ProjectUpdateService, ProjectDeleteService
...
def project_list():
    ...
    response = ProjectListResponse(__root__=[
        ProjectResponse(**result.dict()).dict() for result in results
    ])
...
@api.get(
    "/days/",
    responses={
        "200": ProjectDaysListResponse
    },
)
def project_days_list():
    repository = ProjectRepository
    results = ProjectListService.execute(repository)
    response = ProjectDaysListResponse(__root__=[
        ProjectDaysResponse(**result.dict()).dict() for result in results
    ])
...
def project_create(body: ProjectBody):
    ...
    result = ProjectCreateService.execute(repository, user_repository, company_repository, body)
...
def project_update(body: ProjectBody, path: ProjectUpdatePath):
    ...
    result = ProjectUpdateService.execute(repository, user_repository, company_repository, body, project_id)
...
api.register_error_handler(
    ProjectNotFoundException, project_not_found
)
api.register_error_handler(
